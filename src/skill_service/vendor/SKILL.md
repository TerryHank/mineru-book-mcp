---
name: mineru-book-to-skill
description: "Convert books, PDFs, scans, images, Office files, web pages and ebooks into structured agent skills. Preserves book-to-skill chapter generation, glossary, patterns, cheatsheet, analysis and update workflows, with optional MinerU OCR and layout extraction."
---
...
# MinerU Book-to-Skill Converter
...
For directories and globs, expand matching local formats plus MinerU PDF/image/Office formats. Web URLs require the precision backend; see docs/mineru.md.

If no supported files are found, stop with a clear error message.

---

## Step 1.5 — Choose content and extraction backend

Infer technical versus text-heavy from the document and the user's intended use when clear; ask only if this affects the result and cannot be determined. Existing Markdown uses local reading even in technical mode.

- `local`: original book-to-skill parsers for PDF, EPUB, DOCX, HTML, RTF, text and Calibre ebooks.
- `mineru`: standalone `scripts/mineru.py` from Nebutra/MinerU-Skill. It auto-routes free Agent API and optional Standard API, supports OCR, Office files, images, URLs, batches and `--stdout`.
- `book`: run MinerU first, then feed its Markdown output to book-to-skill for chapters, glossary, patterns and cheatsheet.

MinerU sends selected documents to mineru.net. Disclose this before remote processing. Read [docs/mineru.md](docs/mineru.md) for limits, dependencies and examples.

### ChatGPT attachment boundary

Treat a file attached in a ChatGPT conversation as an attachment reference, not
as a public URL. A remote MinerU MCP server cannot read that attachment unless
the host explicitly exposes a file URL or upload tool. Do not invent a URL, put a
temporary token in a link, open a MinerU upload page as a workaround, or claim
that MCP processed the attachment. If the host exposes a local filesystem path,
use the local MinerU CLI backend. Otherwise report the boundary and ask for an
authorized accessible URL or use the host's native file extraction first, then
feed the resulting Markdown to the local book-to-skill pipeline.

---

## Step 2 — Extract text from the source documents

Resolve `scripts/extract.py` relative to **this SKILL.md**. Do not search for or invoke an independently installed book-to-skill copy.

```text
python "<this-skill>/scripts/extract.py" "<source>" --backend local --mode technical
python "<this-skill>/scripts/mineru.py" "<source>" --stdout
python "<this-skill>/scripts/mineru.py" "<source>" --json
python "<this-skill>/scripts/mineru.py" "<source>" --ocr --lang ch
python "<this-skill>/scripts/extract.py" "<normalized.md>" --backend local --mode technical
```

Use `py` on Windows if `python` is unavailable. Files, folders, globs and mixed input lists are supported. Set `BOOK_SKILL_WORKDIR` to a task-local scratch directory when needed. Each backend run creates a unique child directory.

Read the printed `Meta ->` file and `Text ->` file:
- `full_text.txt`: normalized source text with source boundaries.
- `metadata.json`: original sources, extractor methods, headings, token estimates, and failures.
- `source-NNNN/`: original MinerU Markdown and available extracted assets.

The standalone MinerU `--json` status and the book-to-skill `metadata.json` must be checked before claiming conversion. A failed or partial run returns nonzero; inspect errors and disclose omitted sources. Never treat extraction output as the finished generated skill.

Read the full original Markdown when tables or image links matter. Preserve supplied image assets or disclose missing diagrams; do not equate text extraction with image understanding. HTTP URL query strings and credentials are not persisted in the manifest.

The original local invocation without `--backend` remains compatible. Remote output passes the existing Unicode sanitizer and structure/token analysis. Do not claim either OCR or VLM is error-free.
...
Present the estimate before generation. Continue when the user has already authorized full conversion; ask only for a missing material choice. If they say "analyze only", switch to Mode 2.
...
Before generating, use the user's stated purpose; ask only if it is missing:
...
For a Codex personal install, use `$CODEX_HOME/skills` (default `~/.codex/skills`) unless the user requests another root.
