...
The original directories and local extractor below are retained. Explicit
`--backend local|flash|precision` routes through `book_to_skill/mineru.py`;
the original invocation without that flag still uses the upstream CLI.
MinerU normalizes selected PDFs/images/Office/web sources, then reuses the
same Unicode sanitizer, chapter analysis, and token estimator. It emits
full_text.txt and metadata.json plus per-source Markdown/assets.
The agent still executes SKILL.md to generate chapters, glossary, patterns
and cheatsheet. It is not an automatic Python-based skill generator.

The fused SKILL locates scripts relative to its own folder. The old Bash
host-discovery tests have been replaced with actual relocated-entrypoint
tests across Codex/Agents/Claude/Copilot/Hermes directory layouts.
